...
test_scores = nb.predict_proba(X_test)[:, 1]
...
result_df.to_csv(
    os.path.join(RESULT_DIR, "nb_metrics.csv"),
    index=False
)
...
background_df = X_train.sample(n=background_size, random_state=42)
...
print(os.path.join(RESULT_DIR, "nb_metrics.csv"))
